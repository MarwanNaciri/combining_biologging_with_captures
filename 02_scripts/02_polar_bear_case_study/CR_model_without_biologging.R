...
CR_model <- nimbleCode({
  ...
  for (i in 1:n_ind) {
    ...
    Theta[9, 7, i] <- 0
    Theta[9, 8, i] <- 0
    Theta[9, 9, i] <- 1-ALPHA[i, f[i]]
    Theta[9, 10, i] <- 0
    Theta[9, 11, i] <- ALPHA[i, f[i]]
    Theta[9, 12, i] <- 0
    ...
    Theta[10, 7, i] <- 0
    Theta[10, 8, i] <- 0
    Theta[10, 9, i] <- 2 * ( 1-ALPHA[i, f[i]] ) * ALPHA[i, f[i]]
    Theta[10, 10, i] <- ( 1 - ALPHA[i, f[i]] )^2
    Theta[10, 11, i] <- ALPHA[i, f[i]]^2
    Theta[10, 12, i] <- 0
    ...
    for (t in f[i]:(l[i] - 1)) { 
      ...
      Psi[2, 10, i, t] <- 0
      Psi[2, 11, i, t] <- 0
      Psi[2, 12, i, t] <- 1-phi[i, t]
      
      Psi[3, 1, i, t] <- 0
      Psi[3, 2, i, t] <- 0
      Psi[3, 3, i, t] <- 0
      Psi[3, 4, i, t] <- phi[i, t] * (1- beta[i, t, 1])
      Psi[3, 5, i, t] <- phi[i, t] * beta[i, t, 1] * (1 - gamma[i, t])
      Psi[3, 6, i, t] <- phi[i, t] * beta[i, t, 1] * gamma[i, t]
      Psi[3, 7, i, t] <- 0
      Psi[3, 8, i, t] <- 0
      Psi[3, 9, i, t] <- 0
      Psi[3, 10, i, t] <- 0
      Psi[3, 11, i, t] <- 0
      Psi[3, 12, i, t] <- 1-phi[i, t]
      
      Psi[4, 1, i, t] <- 0
      Psi[4, 2, i, t] <- 0
      Psi[4, 3, i, t] <- 0
      Psi[4, 4, i, t] <- 0
      Psi[4, 5, i, t] <- phi[i, t] * beta[i, t, 1] * (1 - gamma[i, t])
      Psi[4, 6, i, t] <- phi[i, t] * beta[i, t, 1] * gamma[i, t]
      Psi[4, 7, i, t] <- 0
      Psi[4, 8, i, t] <- 0
      Psi[4, 9, i, t] <- 0
      Psi[4, 10, i, t] <- 0
      Psi[4, 11, i, t] <- phi[i, t] * (1 - beta[i, t, 1])
      Psi[4, 12, i, t] <- 1-phi[i, t]
      
      Psi[5, 1, i, t] <- 0
      Psi[5, 2, i, t] <- 0
      Psi[5, 3, i, t] <- 0
      Psi[5, 4, i, t] <- 0
      Psi[5, 5, i, t] <- phi[i, t] * (1-s01[i, t]) * beta[i, t, 2] * (1 - gamma[i, t])
      Psi[5, 6, i, t] <- phi[i, t] * (1-s01[i, t]) * beta[i, t, 2] * gamma[i, t]
      Psi[5, 7, i, t] <- phi[i, t] * s01[i, t]
      Psi[5, 8, i, t] <- 0
      Psi[5, 9, i, t] <- 0
      Psi[5, 10, i, t] <- 0
      Psi[5, 11, i, t] <- phi[i, t] * (1-s01[i, t]) * (1 - beta[i, t, 2])
      Psi[5, 12, i, t] <- 1-phi[i, t]
      
      Psi[6, 1, i, t] <- 0
      Psi[6, 2, i, t] <- 0
      Psi[6, 3, i, t] <- 0
      Psi[6, 4, i, t] <- 0
      Psi[6, 5, i, t] <- phi[i, t] * (1-s02[i, t])^2 * beta[i, t, 2] * (1 - gamma[i, t])
      Psi[6, 6, i, t] <- phi[i, t] * (1-s02[i, t])^2 * beta[i, t, 2] * gamma[i, t]
      Psi[6, 7, i, t] <- phi[i, t] * 2 * s02[i, t] * (1-s02[i, t])
      Psi[6, 8, i, t] <- phi[i, t] * s02[i, t]^2
      Psi[6, 9, i, t] <- 0
      Psi[6, 10, i, t] <- 0
      Psi[6, 11, i, t] <- phi[i, t] * (1-s02[i, t])^2 * (1 - beta[i, t, 2])
      Psi[6, 12, i, t] <- 1-phi[i, t]
      
      Psi[7, 1, i, t] <- 0
      Psi[7, 2, i, t] <- 0
      Psi[7, 3, i, t] <- 0
      Psi[7, 4, i, t] <- 0
      Psi[7, 5, i, t] <- phi[i, t] * (1-s1[i, t]) * beta[i, t, 3] * (1 - gamma[i, t])
      Psi[7, 6, i, t] <- phi[i, t] * (1-s1[i, t]) * beta[i, t, 3] * gamma[i, t]
      Psi[7, 7, i, t] <- 0
      Psi[7, 8, i, t] <- 0
      Psi[7, 9, i, t] <- phi[i, t] * s1[i, t]
      Psi[7, 10, i, t] <- 0
      Psi[7, 11, i, t] <- phi[i, t] * (1-s1[i, t]) * (1 - beta[i, t, 3])
      Psi[7, 12, i, t] <- 1-phi[i, t]
      
      Psi[8, 1, i, t] <- 0
      Psi[8, 2, i, t] <- 0
      Psi[8, 3, i, t] <- 0
      Psi[8, 4, i, t] <- 0
      Psi[8, 5, i, t] <- phi[i, t] * (1-s1[i, t])^2 * beta[i, t, 3] * (1 - gamma[i, t])
      Psi[8, 6, i, t] <- phi[i, t] * (1-s1[i, t])^2 * beta[i, t, 3] * gamma[i, t]
      Psi[8, 7, i, t] <- 0
      Psi[8, 8, i, t] <- 0
      Psi[8, 9, i, t] <- phi[i, t] * 2 * s1[i, t] * (1-s1[i, t])
      Psi[8, 10, i, t] <- phi[i, t] * s1[i, t]^2
      Psi[8, 11, i, t] <- phi[i, t] * (1-s1[i, t])^2 * (1 - beta[i, t, 3])
      Psi[8, 12, i, t] <- 1-phi[i, t]
      
      Psi[9, 1, i, t] <- 0
      Psi[9, 2, i, t] <- 0
      Psi[9, 3, i, t] <- 0
      Psi[9, 4, i, t] <- 0
      Psi[9, 5, i, t] <- phi[i, t] * beta[i, t, 4] * (1 - gamma[i, t])
      Psi[9, 6, i, t] <- phi[i, t] * beta[i, t, 4] * gamma[i, t]
      Psi[9, 7, i, t] <- 0
      Psi[9, 8, i, t] <- 0
      Psi[9, 9, i, t] <- 0
      Psi[9, 10, i, t] <- 0
      Psi[9, 11, i, t] <- phi[i, t] * (1 - beta[i, t, 4])
      Psi[9, 12, i, t] <- 1-phi[i, t]
      
      Psi[10, 1, i, t] <- 0
      Psi[10, 2, i, t] <- 0
      Psi[10, 3, i, t] <- 0
      Psi[10, 4, i, t] <- 0
      Psi[10, 5, i, t] <- phi[i, t] * beta[i, t, 4] * (1 - gamma[i, t])
      Psi[10, 6, i, t] <- phi[i, t] * beta[i, t, 4] * gamma[i, t]
      Psi[10, 7, i, t] <- 0
      Psi[10, 8, i, t] <- 0
      Psi[10, 9, i, t] <- 0
      Psi[10, 10, i, t] <- 0
      Psi[10, 11, i, t] <- phi[i, t] * (1 - beta[i, t, 4])
      Psi[10, 12, i, t] <- 1-phi[i, t]
      
      Psi[11, 1, i, t] <- 0
      Psi[11, 2, i, t] <- 0
      Psi[11, 3, i, t] <- 0
      Psi[11, 4, i, t] <- 0
      Psi[11, 5, i, t] <- phi[i, t] * beta[i, t, 1] * (1 - gamma[i, t])
      Psi[11, 6, i, t] <- phi[i, t] * beta[i, t, 1] * gamma[i, t]
      Psi[11, 7, i, t] <- 0
      Psi[11, 8, i, t] <- 0
      Psi[11, 9, i, t] <- 0
      Psi[11, 10, i, t] <- 0
      Psi[11, 11, i, t] <- phi[i, t] * (1 - beta[i, t, 1])
      Psi[11, 12, i, t] <- 1-phi[i, t]
      ...
      Xi[1, 10, i, t] <- 0
      Xi[1, 11, i, t] <- 0
      Xi[1, 12, i, t] <- 1-p[i, t+1, 1]
      
      Xi[2, 1, i, t] <- 0
      Xi[2, 2, i, t] <- p[i, t+1, 1]
      Xi[2, 3, i, t] <- 0
      Xi[2, 4, i, t] <- 0
      Xi[2, 5, i, t] <- 0
      Xi[2, 6, i, t] <- 0
      ...
      Xi[2, 10, i, t] <- 0
      Xi[2, 11, i, t] <- 0
      Xi[2, 12, i, t] <- 1 - p[i, t+1, 1]
      
      Xi[3, 1, i, t] <- 0
      Xi[3, 2, i, t] <- 0
      Xi[3, 3, i, t] <- p[i, t+1, 1]
      Xi[3, 4, i, t] <- 0
      Xi[3, 5, i, t] <- 0
      Xi[3, 6, i, t] <- 0
      Xi[3, 7, i, t] <- 0
      ...
      Xi[3, 10, i, t] <- 0
      Xi[3, 11, i, t] <- 0
      Xi[3, 12, i, t] <- 1 - p[i, t+1, 1]
      
      Xi[4, 1, i, t] <- 0
      Xi[4, 2, i, t] <- 0
      Xi[4, 3, i, t] <- 0
      Xi[4, 4, i, t] <- p[i, t+1, 1]
      Xi[4, 5, i, t] <- 0
      Xi[4, 6, i, t] <- 0
      Xi[4, 7, i, t] <- 0
      Xi[4, 8, i, t] <- 0
      ...
      Xi[4, 10, i, t] <- 0
      Xi[4, 11, i, t] <- 0
      Xi[4, 12, i, t] <- 1 - p[i, t+1, 1]
      
      Xi[5, 1, i, t] <- 0
      Xi[5, 2, i, t] <- 0
      Xi[5, 3, i, t] <- 0
      Xi[5, 4, i, t] <- 0
      Xi[5, 5, i, t] <- p[i, t+1, 2]
      Xi[5, 6, i, t] <- 0
      Xi[5, 7, i, t] <- 0
      Xi[5, 8, i, t] <- 0
      Xi[5, 9, i, t] <- 0
      ...
      Xi[6, 4, i, t] <- 0
      Xi[6, 5, i, t] <- 0
      Xi[6, 6, i, t] <- p[i, t+1, 2]
      Xi[6, 7, i, t] <- 0
      Xi[6, 8, i, t] <- 0
      Xi[6, 9, i, t] <- 0
      Xi[6, 10, i, t] <- 0
      Xi[6, 11, i, t] <- 0
      Xi[6, 12, i, t] <- 1 - p[i, t+1, 2]
      ...
      Xi[7, 5, i, t] <- 0
      Xi[7, 6, i, t] <- 0
      Xi[7, 7, i, t] <- p[i, t+1, 1]
      Xi[7, 8, i, t] <- 0
      Xi[7, 9, i, t] <- 0
      Xi[7, 10, i, t] <- 0
      Xi[7, 11, i, t] <- 0
      Xi[7, 12, i, t] <- 1 - p[i, t+1, 1]
      ...
      Xi[8, 6, i, t] <- 0
      Xi[8, 7, i, t] <- 0
      Xi[8, 8, i, t] <- p[i, t+1, 1]
      Xi[8, 9, i, t] <- 0
      Xi[8, 10, i, t] <- 0
      Xi[8, 11, i, t] <- 0
      Xi[8, 12, i, t] <- 1 - p[i, t+1, 1]
      ...
      Xi[9, 7, i, t] <- 0
      Xi[9, 8, i, t] <- 0
      Xi[9, 9, i, t] <- (1 - ALPHA[i, t+1]) * p[i, t+1, 1]
      Xi[9, 10, i, t] <- 0
      Xi[9, 11, i, t] <- ALPHA[i, t+1] * p[i, t+1, 1]
      Xi[9, 12, i, t] <- 1 - p[i, t+1, 1]
      ...
      Xi[10, 7, i, t] <- 0
      Xi[10, 8, i, t] <- 0
      Xi[10, 9, i, t] <- 2 * (1 - ALPHA[i, t+1]) * ALPHA[i, t+1] * p[i, t+1, 1]
      Xi[10, 10, i, t] <- (1 - ALPHA[i, t+1])^2 * p[i, t+1, 1]
      Xi[10, 11, i, t] <- ALPHA[i, t+1]^2 * p[i, t+1, 1]
      Xi[10, 12, i, t] <- 1 - p[i, t+1, 1]
      ...
      Xi[11, 9, i, t] <- 0
      Xi[11, 10, i, t] <- 0
      Xi[11, 11, i, t] <- p[i, t+1, 1]
      Xi[11, 12, i, t] <- 1 - p[i, t+1, 1]
