...
CR_model_biologging_sim <- nimbleCode({
  ...
  # delta: breeding probability * early offspring survival (beta * eta)
  # gamma: probability of a large litter/brood

  # -------------------------------------------------+
  # States (z):
  # 1 = J: Juvenile (shortly before independence)
  # 2 = L: Lone
  ...
  # -------------------------------------------------+
  # Events (y):
  # 1 = J: Capture of a juvenile
  # 2 = L: Capture of a lone individual 
  # 3 = SB1: Capture of an individual with litter/brood size = 1
  # 4 = SB2: Capture of an individual with litter/brood size = 2
  # 5 = NA: Not captured/observed
  ...
  for (t in 1:(n_occasions - 1)) {
    ...
    S[1, 1, t] <- 0
    S[1, 2, t] <- phi[1, t] * (1 - delta[1])
    S[1, 3, t] <- phi[1, t] * delta[1] * (1 - gamma)
    S[1, 4, t] <- phi[1, t] * delta[1] * gamma
    S[1, 5, t] <-  1-phi[1, t]
    
    S[2, 1, t] <- 0
    S[2, 2, t] <- phi[2, t] * (1 - delta[2])
    S[2, 3, t] <- phi[2, t] * delta[2] * (1 - gamma)
    S[2, 4, t] <- phi[2, t] * delta[2] * gamma
    S[2, 5, t] <-  1-phi[2, t]
    
    S[3, 1, t] <- 0
    S[3, 2, t] <- phi[2, t] * (1 - delta[3])
    S[3, 3, t] <- phi[2, t] * delta[3] * (1 - gamma)
    S[3, 4, t] <- phi[2, t] * delta[3] * gamma
    S[3, 5, t] <-  1-phi[2, t]
    
    S[4, 1, t] <- 0
    S[4, 2, t] <- phi[2, t] * (1 - delta[3])
    S[4, 3, t] <- phi[2, t] * delta[3] * (1 - gamma)
    S[4, 4, t] <- phi[2, t] * delta[3] * gamma
    S[4, 5, t] <-  1-phi[2, t]
  ...
  delta[1] ~ dunif(0, 1)  # successful breeding probability (J)
  delta[2] ~ dunif(0, 1)  # successful breeding probability (NB/FB)
  delta[3] ~ dunif(0, 1)  # successful breeding probability (SB)
  ...
  for (l in 1:length(values_pGPS)) {
    for (m in 1:length(values_pError)) {
      for (n in n_sim_start:n_sim) {
        ...
        #                        J           L        SB1        SB2             NA
        E <- matrix(data = c(values_p[k],    0,        0,         0,       1-values_p[k],  # J 
                                 0,      values_p[k],  0,         0,       1-values_p[k],  # NB
                                 0,      values_p[k],  0,         0,       1-values_p[k],  # FB
                                 0,          0,     values_p[k],  0,       1-values_p[k],  # SB1
                                 0,          0,        0,    values_p[k],  1-values_p[k],  # SB2
                                 0,          0,        0,         0,            1),        # D
                      nrow = n_states_simul, ncol = n_events, byrow = TRUE)
        
        #                     J    L   SB1  SB2  D
        E0 <- matrix(data = c(1,   0,   0,   0,  0,  # J
                              0,   1,   0,   0,  0,  # NB
                              0,   1,   0,   0,  0,  # FB
                              0,   0,   1,   0,  0,  # SB1
                              0,   0,   0,   1,  0,  # SB2
        ...
        for (i in 1:(new_per_year*n_occasions - new_per_year)) {
          ...
          CH[i, f[i]] <- which(rmultinom(n = 1, size = 1, 
                                         prob = E0[Z[i, f[i]], ]) == 1)
        ...
        # Save
        name <- paste0("03_outputs/simulations/simulated_datasets/rock_partridge/no_biologging/",
                       "p_", values_p[k],
