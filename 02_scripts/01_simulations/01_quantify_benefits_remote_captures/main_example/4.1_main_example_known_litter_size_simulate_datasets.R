...
CR_model_biologging_sim <- nimbleCode({
  ...
  for (t in 1:(n_occasions - 1)) {
    ...
    for (i in 1:n_ind) {
      ...
      E[6, 3, i, t] <- 0
      E[6, 4, i, t] <- 0
      E[6, 5, i, t] <- p + (1 - p) * KHI[i, t+1]
      E[6, 6, i, t] <- 0
      E[6, 7, i, t] <- 0
      E[6, 8, i, t] <- (1 - p) * (1 - KHI[i, t+1])
...
p <- pi <- 0.5
#                    F       Y       L        SB1      SB2     NB       FB           D
E <- matrix(data = c(p,      0,      0,        0,       0,     0,       0,          1-p,       # F
                     0, p+(1-p)*pi,  0,        0,       0,     0,       0,      (1-p)*(1-pi),  # Y
                     0,      0,   p*(1-pi),    0,       0,     pi,      0,      (1-p)*(1-pi),  # NB
                     0,      0,   p*(1-pi),    0,       0,     0,       pi,     (1-p)*(1-pi),  # FB
                     0,      0,      0,    p+(1-p)*pi,  0,     0,       0,      (1-p)*(1-pi),  # SB1
                     0,      0,      0,        0,  p+(1-p)*pi, 0,       0,      (1-p)*(1-pi),  # SB2
                     0,      0,      0,        0,       0,     0,       0,           1),       # D
  ...
  for (l in 1:length(values_pGPS)) {
    for (m in 1:length(values_pError)) {
      for (n in n_sim_start:n_sim) {
        ...
        #                     F  Y  L  SB1 SB2  NB  FB  NA
        E0 <- matrix(data = c(1, 0, 0,  0,  0,  0,  0,  0,  # F 
                              0, 1, 0,  0,  0,  0,  0,  0,  # Y 
                              0, 0, 1,  0,  0,  0,  0,  0,  # NB 
                              0, 0, 1,  0,  0,  0,  0,  0,  # FB 
                              0, 0, 0,  1,  0,  0,  0,  0,  # SB1 
          ...
          for (t in f[i]:(n_occasions-1)) {
            ...
            if (values_pError[m] != 0) {
              ...
              if (p_realized[i, t+1] == 1) { # If physically captured 
                ...
                E[2, 6, i, t] <- 0
                E[2, 7, i, t] <- 0
                E[2, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
                ...
                E[3, 6, i, t] <- KHI[i, t+1]
                E[3, 7, i, t] <- 0
                E[3, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
                ...
                E[4, 4, i, t] <- 0                                           # SB1
                E[4, 5, i, t] <- 0                                           # SB2
                E[4, 6, i, t] <- KHI[i, t+1] * values_pError[m]              # NB
                E[4, 7, i, t] <- KHI[i, t+1] * (1- values_pError[m])         # FB
                E[4, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
                
                # SB1 cannot be erroneously categorized when physical captures occur.
                E[5, 1, i, t] <- 0
                E[5, 2, i, t] <- 0
                E[5, 3, i, t] <- 0
                E[5, 4, i, t] <- p_realized[i, t+1] + (1 - p_realized[i, t+1]) * KHI[i, t+1]
                E[5, 5, i, t] <- 0
                E[5, 6, i, t] <- 0
                E[5, 7, i, t] <- 0
                E[5, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
                ...
                E[6, 3, i, t] <- 0
                E[6, 4, i, t] <- 0
                E[6, 5, i, t] <- p_realized[i, t+1] + (1 - p_realized[i, t+1]) * KHI[i, t+1]
                E[6, 6, i, t] <- 0
                E[6, 7, i, t] <- 0
                E[6, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
                ...
                E[5, 2, i, t] <- 0
                E[5, 3, i, t] <- 0
                E[5, 4, i, t] <- p_realized[i, t+1] + (1 - p_realized[i, t+1]) * KHI[i, t+1] * (1 - values_pError[m])
                E[5, 5, i, t] <- 0
                E[5, 6, i, t] <- KHI[i, t+1] * values_pError[m]              # NB (i.e. erroneously categorized as non-breeder)
                E[5, 7, i, t] <- 0
                E[5, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
                ...
                E[6, 4, i, t] <- 0
                E[6, 5, i, t] <- p_realized[i, t+1] + (1 - p_realized[i, t+1]) * KHI[i, t+1]
                E[6, 6, i, t] <- KHI[i, t+1] * values_pError[m]              # NB (i.e. erroneously categorized as non-breeder)
                E[6, 7, i, t] <- 0
                E[6, 8, i, t] <- (1 - p_realized[i, t+1]) * (1 - KHI[i, t+1])
        ...
        #                    C  Y  L  SB1 SB2  NB   FB  NA
        E <- matrix(data = c(1, 0, 0,  0,  0,  0,   0,  1,  # C
                             0, 1, 0,  0,  0,  0,   0,  1,  # Y
                             0, 0, 1,  0,  0,  1,   0,  1,  # NB 
                             0, 0, 1,  0,  0,  0,   1,  1,  # FB 
                             0, 0, 0,  1,  0,  0,   0,  1,  # SB1 
        ...
        # Save
        name <- paste0("03_outputs/simulations/simulated_datasets/main_example/with_biologging/known_litter_size/",
                       "p_", values_p[k],
